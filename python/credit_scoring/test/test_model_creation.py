...
try:
    ...
    log.error(
        "Este script de test espera estar en una carpeta 'tests/' y el modelo en 'src/training/model.py'"
    )
    sys.exit(1)
...
def test_model_architecture(model_config_fixture):
    ...
    assert (
        len(net) == expected_layers_count
    ), f"Se esperaban {expected_layers_count} capas, pero se encontraron {len(net)}."
...
def test_forward_pass(model_config_fixture):
    ...
    ), f"La forma del tensor de salida es incorrecta. Esperado: {expected_shape}, Obtenido: {output.shape}"
    log.info(
        f"✔ ¡Éxito! El forward pass se completó y la forma de salida es correcta: {output.shape}."
    )
