...
class CreditScoringModelTraining:
    def __init__(self, config_path: Path) -> None:
        ...
        self.dataset_path = Path(
            self.params["data_source"]["data_path"]["dataset_path"]
        )
        self.artifact_name_or_path = self.params["data_source"]["data_path"][
            "artifact_path"
        ]
        self.preprocessor_filename = self.params["data_source"]["data_path"][
        ...
        self.weight_decay = train_cfg["optimizer"].get(
            "weight_decay", 0.0
        )  # Default a 0
    ...
    def _preprocess_data(
        ...
        x_train_processed, y_train = self.data_preprocessor.process_data(
            df_train, preprocessor
        )
        x_val_processed, y_val = self.data_preprocessor.process_data(
            df_val, preprocessor
        )
    ...
    def _evaluate_split(
        self, model, x: torch.Tensor, y: torch.Tensor, criterion
    ) -> Dict[str, float]:
        """
    ...
    def _plot_confusion_matrix(
        self, y_true: np.ndarray, y_pred: np.ndarray, filename: str
    ):
    ...
    def _plot_roc_pr(
        self, y_true: np.ndarray, y_prob: np.ndarray, roc_file: str, pr_file: str
    ):
        # ROC
        try:
    ...
    def _run_training_loop(
        self, model, criterion, optimizer, scheduler, x_train, y_train, x_val, y_val
    ):
    ...
    def _generate_and_log_performance_report(
        ...
        report_data = {
            "benchmark_id": self.params.get("project_info", {}).get(
                "benchmark_id", "N/A"
            ),
            ...
            "final_validation_metrics": {
                k: round(v, 4) for k, v in final_metrics.items() if not math.isnan(v)
            },
        }
    ...
    def _log_plots_and_reports(self, y_true_val: np.ndarray, y_prob_val: np.ndarray):
        ...
        roc_png, pr_png = self._plot_roc_pr(
            y_true_val, y_prob_val, roc_file="roc_val.png", pr_file="pr_val.png"
        )
        y_pred_val = (y_prob_val >= 0.5).astype(int)
        cm_png = self._plot_confusion_matrix(
            y_true_val, y_pred_val, filename="confusion_matrix_val.png"
        )

        # Classification report como archivo de texto
        cls_report = classification_report(
            y_true_val, y_pred_val, target_names=["bad(0)", "good(1)"]
        )
    ...
    def _setup_loss_function(self, y_train: torch.Tensor) -> nn.Module:
        ...
        if self.use_pos_weight:
            ...
            if pos == 0 or neg == 0:
                log.warning(
                    "Una de las clases no está presente en el batch de entrenamiento. No se usará pos_weight."
                )
                return nn.BCEWithLogitsLoss()

            pos_weight_value = neg / pos
            pos_weight_tensor = torch.tensor(
                [pos_weight_value], dtype=torch.float32, device=self.device
            )
            log.info(
                f"✔ using weighted BCE loss with pos_weight={pos_weight_value:.4f}"
            )
    ...
    def train(self):
        ...
        run_name_prefix = self.params["mlflow_config"].get(
            "mlflow_run_name_prefix", "credit_scoring_run"
        )
        ...
        with mlflow.start_run(run_name=f"{run_name_prefix}"):
            ...
            log.info(
                f"✔ using optimizer: {self.optimizer_name} with lr={self.learning_rate}"
            )
            ...
            epochs_run = self._run_training_loop(
                model, criterion, optimizer, scheduler, x_train, y_train, x_val, y_val
            )
            ...
            self._generate_and_log_performance_report(
                model, final_metrics, num_features, epochs_run, run_name_prefix
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Threshold optimizer for ID classification model."
    )
    parser.add_argument(
        "--config",
        type=str,
...
"""
execute training file by command:
